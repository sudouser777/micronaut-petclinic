...
public class OwnerResourceClientController {
    ...
    @NewSpan
    @Get("/owners")
    public HttpResponse<List<OwnerDTO>> getAllOwners(@SpanTag("http.request") HttpRequest<?> request, Pageable pageable) {
    ...
    @NewSpan
    @Delete("/owners/{id}")
    HttpResponse deleteOwner(@SpanTag("owner.id") @PathVariable Long id) {
